   ...
   return {
      ...
      group: async (id, update = {}) => {
         try {
            ...
            console.error('❌ Failed to fetch group metadata', ':', id, '-', error?.message || error)
            return true
         }
      },
      message: async (message) => {
         ...
         const isOwner = m.fromMe ||
            ...
            (m.isGroup && m.sender.endsWith(LID) && await sock.findUserId(m.sender).then((result) => result?.phoneNumber?.startsWith(ownerNumber)).catch(() => false))
         ...
         if (plugin) {
            ...
            if (plugin.private && !m.isPrivate && !isOwner)
               return
